...
def read_infile(infile):
    ...
    try:
        ...
        print(f'CRITICAL: Error reading status file {infile} ({type(ex).__name__} – {ex})')
        sys.exit(2)
...
def analyze_content(content):
    ...
    if delta < 0:
        print(f'CRITICAL: Check script claims to be executed in future ({content[0]})??')
        sys.exit(2)

    if delta > max_delta_crit:
        print(f'CRITICAL: Last check script run was {delta} sec ago – check your cron job.')
        sys.exit(2)

    if delta > max_delta_warn:
        print(f'WARNING: Last check script run was {delta} sec ago – check your cron job.')
        sys.exit(1)
