...
class DotvimPopulator:
    ...
    def _check_args(self):
        ...
        def error_func():
            print()
            print(
                "Usage: {} [{}]".format(sys.argv[0], "|".join(self.tasks))
                + " {opt_git_cmd}"
            )
            print(
                "           as second argument you may pass the path to a (different) git executable"
            )
            print()
            sys.exit(1)
    ...
    def _install_pathogen(self):
        ...
        with urllib.request.urlopen("https://tpo.pe/pathogen.vim") as response, open(
            os.path.join(self.vim_dir, "autoload", "pathogen.vim"), "wb"
        ) as out_file:
    ...
    def _install_plugin(self, name, source):
        ...
        subprocess.call(
            [self.git_cmd, "clone", source, os.path.join(self.bundle_dir, name)]
        )
